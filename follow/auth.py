...
def _get_auth_stub() -> auth_pb2_grpc.AuthServiceStub:
    ...
    if _stub is None:
        ...
        logger.info("Follow→Auth gRPC channel opened at %s", addr)
    return _stub
...
async def get_current_user_id(
    ...
    if not response.valid:
        logger.warning("Token rejected by Auth service: %s", response.error)
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
    ...
    user_id: int = response.user.id
    logger.debug("Authenticated user_id=%s via gRPC", user_id)
    return user_id
